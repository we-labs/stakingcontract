...
contract StakingSkull is Ownable, ReentrancyGuard{
    ...
    // Staking Lands
    function stakeLands(uint256[] calldata _tokensIds) external {
    ...
    function unstakeLands(uint256[] calldata _tokensIds) external {
    ...
    // Staking Juniors
    function stakeJuniors(uint256[] calldata _tokensIds) external {
        for (uint256 i = 0; i < _tokensIds.length; i++) {
    ...
    function unstakeJuniors(uint256[] calldata _tokensIds) external {
        for (uint256 j = 0; j < _tokensIds.length; j++) {
    ...
    // Unstake all
    function unstakeAllLands() public onlyOwner {
        for (uint256 _tokenId = 0; _tokenId < landSupply; _tokenId++) {
            for (uint256 _index = 0; _index < stakers[landAddress[_tokenId]].stakedLands.length; _index++) {
    ...
    function unstakeAllJuniors() public onlyOwner {
        for (uint256 _tokenId = 0; _tokenId < juniorSupply; _tokenId++) {
            for (uint256 _index = 0; _index < stakers[juniorAddress[_tokenId]].stakedJuniors.length; _index++) {
                ...
                juniorContract.transferFrom(address(this), juniorAddress[_tokenId], _tokenId);
